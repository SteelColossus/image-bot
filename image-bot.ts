...
if (
    ...
    logger.error(
        'One or more of the required API keys were not found. Have you set the correct environment variables?',
    );
    process.exit(1);
}
...
async function postRandomImage<T>(
    query: string,
    sendMessage: (message: BaseMessageOptions | string) => Promise<T>,
    ...
    try {
        const images = await imageClient.search(query, {
            page: pageNumber,
            safe: safeSetting,
        });
        ...
        if (images.length > 0) {
            ...
            while (
                !(
                    ((image.url.endsWith('.jpg') || image.url.endsWith('.png')) && !gifsOnly) ||
...
async function postRandomImageWithInteraction(
    query: string,
...
client.on(Events.MessageCreate, async (message) => {
    ...
    if (!message.author.bot && message.content.length > 0) {
        if (autoChannels.get(message.channel.id) ?? false) {
            logger.info(
                `User '${message.author.username}' requested the image '${message.content}' as an automatic image`,
            );
    ...
    switch (interaction.commandName) {
        case 'enableautoimages': {
            ...
            if (!alreadyEnabled) {
                ...
                logger.info(
                    `Automatic images were enabled in channel: ${getChannelName(interaction.channel)}, id: ${channelId}`,
                );
            } else {
        ...
        case 'disableautoimages': {
            ...
            if (!alreadyDisabled) {
                ...
                logger.info(
                    `Automatic images were disabled in channel: ${getChannelName(interaction.channel)}, id: ${channelId}`,
                );
            } else {
        ...
        case '8ball': {
            ...
            if (question != null) {
                logger.info(
                    `User '${interaction.user.username}' wanted to know the magic 8-ball's answer to the following question: '${question}'`,
                );
                ...
                await interaction.reply(
                    'The magic 8-ball does not understand what you want - you need to give it a question!',
                );
            }
